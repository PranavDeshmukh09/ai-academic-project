    ...
    <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="px-8 pb-8 relative">
          <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
            <div className="flex flex-col sm:flex-row items-start sm:items-end gap-4">
              {/* Avatar */}
              <div className="-mt-12 w-24 h-24 bg-white rounded-2xl p-1 shadow-md flex-shrink-0 z-10">
              ...
              <div className="pt-2 sm:pt-0">
                <h1 className="text-2xl font-bold text-slate-900">{userProfile?.fullName || 'Student Name'}</h1>
                <p className="text-slate-500 font-medium">{userProfile?.email || 'student@university.edu'}</p>
              </div>
            </div>
            
            <button className="px-4 py-2 bg-slate-100 text-slate-700 hover:bg-slate-200 font-bold text-sm rounded-xl transition-colors cursor-pointer self-start sm:self-auto mt-4 sm:mt-0">
              Edit Profile
            </button>
          </div>
          ...
          <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
              <div>
                ...
                <div className="mt-1 text-slate-800 font-medium">Student</div>
              </div>
              <div>
                <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">Department</label>
                <div className="mt-1 text-slate-800 font-medium">{userProfile?.department || 'Not specified'}</div>
              </div>
              <div>
                <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">Year of Study</label>
                <div className="mt-1 text-slate-800 font-medium">Year {userProfile?.year || '1'}</div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="mt-1 flex items-center space-x-2">
                  ...
                  <span className="text-slate-800 font-medium">Mentor Name (Pending)</span>
                </div>
              </div>
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {userProfile?.skills && userProfile.skills.length > 0 ? (
                    userProfile.skills.map(skill => (
                      <span key={skill} className="px-2 py-1 bg-blue-50 text-[#0252CD] text-xs font-bold rounded-lg border border-blue-100 animate-in fade-in duration-350">
                        {skill}
                      </span>
                    ))
                  ) : (
      ...
      {isEditingSkills && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl border border-slate-200 shadow-2xl w-full max-w-md p-6 overflow-hidden relative animate-in zoom-in-95 duration-200">
            <div className="space-y-4">
              <div className="flex justify-between items-center pb-2 border-b border-slate-100">
                <h3 className="font-extrabold text-slate-800 text-base">Edit Assessed Skills</h3>
                <button 
              ...
              <form onSubmit={handleAddSkill} className="flex gap-2">
                <input
                  ...
                  className="flex-1 bg-slate-100 border-none rounded-xl px-4 py-2 text-xs focus:ring-2 focus:ring-[#0252CD] focus:bg-white transition-all outline-none"
                />
                <button
                  type="submit"
              ...
              <div className="space-y-1">
                ...
                <div className="border border-slate-200 rounded-2xl p-4 bg-slate-50/50 min-h-[100px] max-h-[200px] overflow-y-auto flex flex-wrap gap-2">
                  {editedSkills.length > 0 ? (
                    editedSkills.map(skill => (
                      <span key={skill} className="px-2.5 py-1.5 bg-white text-slate-700 text-xs font-bold rounded-xl border border-slate-250 shadow-sm flex items-center space-x-1.5 animate-in scale-in duration-200">
                        <span>{skill}</span>
                        <button
                          type="button"
              ...
              <div className="flex gap-3 pt-3 border-t border-slate-100 justify-end">
                <button
                  type="button"
                  onClick={() => setIsEditingSkills(false)}
                  className="px-4 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded-xl text-xs transition-colors cursor-pointer border-none"
                  disabled={saving}
                >
                  Cancel
                </button>
