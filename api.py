...
app.include_router(feasibility_agent.router)
app.include_router(planning_agent.router)
app.include_router(scope_agent.router)
app.include_router(techstack_agent.router)
...
def chat(request: ChatInput):
    ...
    raw_reply = res.get("chat_reply", "")
    chat_text = raw_reply
    import json
    try:
        parsed_reply = json.loads(raw_reply, strict=False)
        chat_text = parsed_reply.get("reply", raw_reply)
    except Exception:
        pass
